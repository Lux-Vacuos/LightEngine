 ...
 */
package net.guerra24.infinity.client.sound.openal;
...
public final class EFXUtil {
	...
	/**
	 * Tests OpenAL to see whether the given effect type is supported. This is
	 * done by creating an effect of the given type. If creation succeeds the
	 * effect is supported.
	 *
	 * @param effectType
	 *            Type of effect whose support is to be tested, e.g.
	 *            AL_EFFECT_REVERB.
	 *
	 * @return True if it is supported, false if not.
	 *
	 * @throws org.lwjgl.openal.OpenALException
	 *             If the request fails due to an AL_OUT_OF_MEMORY error or
	 *             OpenAL has not been created yet.
	...
	/**
	 * Tests OpenAL to see whether the given filter type is supported. This is
	 * done by creating a filter of the given type. If creation succeeds the
	 * filter is supported.
	 *
	 * @param filterType
	 *            Type of filter whose support is to be tested, e.g.
	 *            AL_FILTER_LOWPASS.
	 *
	 * @return True if it is supported, false if not.
	 *
	...
	/**
	 * Generic test function to see if an EFX object supports a given kind of
	 * type. Works for effects and filters.
	 *
	 * @param objectType
	 *            Type of object to test. Must be either EFXUtil.EFFECT or
	 *            EFXUtil.FILTER.
	 * @param typeValue
	 *            OpenAL type the object should be tested for support, e.g.
	...
	private static boolean testSupportGeneric(int objectType, int typeValue) {
		...
		if (isEfxSupported()) {
			...
			try {
				...
				// Hack because OpenALException hides the original error code
				// (short of parsing the
			...
			if (genError == AL_NO_ERROR) {
				...
				try {
					...
					// Hack because OpenALException hides the original error
					// code (short of parsing
					// the error message String which would break when it gets
					// changed).
