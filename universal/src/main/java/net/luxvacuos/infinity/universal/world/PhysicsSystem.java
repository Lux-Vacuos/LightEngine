...
package net.luxvacuos.infinity.universal.world;
	...
	public void addedToEngine(Engine engine) {
		...
		engine.addEntityListener(new EntityListener() {
			...
			public void entityRemoved(Entity entity) {
				if (entity.getComponent(AABB.class) != null)
					boxes.remove(entity.getComponent(AABB.class).getBoundingBox());
			}

			@Override
			public void entityAdded(Entity entity) {
				if (entity.getComponent(AABB.class) != null)
					boxes.add(entity.getComponent(AABB.class).getBoundingBox());
			}
		});
	}
		...
		for (Entity entity : entities) {
			...
			if (aabb.isEnabled())
				for (BoundingBox boundingBox : boxes) {
					if(aabb.getBoundingBox().equals(boundingBox))
						continue;
					normalTMP.set(0, 0, 0);
